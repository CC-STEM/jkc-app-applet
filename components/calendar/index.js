...
Component({
  ...
  properties: {
    // 底下需要展示小圆点的日期数组
    spot: {
      type: Array, // ['2022/5/6', '2022/5/9', '2022/5/20', '2022/5/12', '2022/6/1'],
      value: []
    },
    // 组件渲染时默认选中的时间
    defaultDate: {
  ...
  methods: {
    // 回到今天
    onSetToday (e) {
      const now = this.data.defaultDate ? dayjs(this.data.defaultDate) : dayjs()
    ...
    dateListInit (paramYear = this.data.selectDay.year, paramMonth = this.data.selectDay.month) {
      ...
      const endDate = dayjs(`${paramYear}-${paramMonth + 1}`).endOf('month').day(6) // 日历主体渲染结束日期
